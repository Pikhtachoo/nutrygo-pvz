...
(function () {
  ...
  (function нашВходВОкне() {
    ...
    function встроить() {
      ...
      // Ставим сразу за формой, а не в конец её родителя: в конце оказалось
      // ниже видимой части окна — блок был отрисован, но не виден
      // (замер: содержимое 997 точек при высоте окна 778).
      if (форма && форма.parentElement) {
        форма.parentElement.insertBefore(обёртка, форма.nextSibling);
      } else {
        д.body.appendChild(обёртка);
      }
    }
