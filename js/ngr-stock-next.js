...
(function () {
  ...
  function подмешатьСвои(текст, url) {
    ...
    if (снимокПолон) {
      живые.forEach(function (p) {
        ...
        if (о == null) return;
        /*
         * Нуль у Tilda не переписываем — иначе покупатель попадает в тупик.
         *
         * 17.08 я подставил живой остаток Ozon всем товарам, и товар с нулём
         * в базе Tilda стал доступен к покупке. Но при оформлении Tilda
         * проверяет остаток на своём сервере и отказала: «отсутствует на складе
         * в необходимом количестве (осталось: 0)» — покупатель заполнил всю
         * форму впустую. Значит показывать можно только то, что Tilda
         * действительно продаст: её собственный нуль оставляем как есть, а
         * число уточняем лишь там, где товар у неё есть.
         *
         * Настоящее лечение — обновлять остатки в самой Tilda; до тех пор
         * такие товары честно скрыты, а не заманивают в отказ.
         */
        var своё = Number(String(p.quantity == null ? '' : p.quantity).replace(',', '.'));
        if (!(своё > 0)) return;
        var строка = String(о);
        p.quantity = строка;
  ...
  function initSmartSearch() {
    ...
    inp.addEventListener('input', function () {
      ...
      /*
       * Во время набора ничего не считаем.
       *
       * Раньше через 280 мс после каждой буквы поднимался указатель на 1340
       * товаров и пересчитывался вес каждого — на телефоне и на слабой машине
       * сайт от этого подвисал (замечание Александра 17.08: «набираешь и сайт
       * виснет»). Теперь поиск начинается только по «Найти» или Enter.
       */
      hide();
      if (сейчас) return;
      /*
