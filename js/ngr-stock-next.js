...
(function () {
   ...
   */
  /*
   * В имени ключа стоит версия.
   *
   * Список товаров и цены хранятся в памяти браузера на полчаса, и Ctrl+F5 их
   * не чистит: 17.08 из-за этого на плитке была наша расчётная цена 802, а в
   * карточке цена Tilda 761 — покупатель видел два числа на один товар.
   * Меняем правило цен — меняем и версию ключа: прежние данные перестают
   * читаться у всех сразу, без просьб «очистите кеш».
   */
  var ВЕРСИЯ_ЗАПАСА = 'v2';
  var КАРТОЧКИ_КЛЮЧ = 'ngr:cards:' + ВЕРСИЯ_ЗАПАСА;
  var ОСТАТКИ_КЛЮЧ = 'ngr:stockmap:' + ВЕРСИЯ_ЗАПАСА;
  ...
  /*
   * Прошлые запасы вычищаем один раз при загрузке: иначе они просто лежат
   * мёртвым грузом в памяти браузера и путают при разборе.
   */
  (function убратьСтарыеЗапасы() {
    try {
      ['ngr:cards', 'ngr:stockmap', 'ngr:prices'].forEach(function (к) { localStorage.removeItem(к); });
    } catch (e) {}
  })();
   ...
   */
  var ЦЕНЫ_КЛЮЧ = 'ngr:prices:' + ВЕРСИЯ_ЗАПАСА;
  var НЕВЕЗЁМ_КЛЮЧ = 'ngr:banned:' + ВЕРСИЯ_ЗАПАСА;
  var НЕ_ВЕЗЁМ = (изПамяти(НЕВЕЗЁМ_КЛЮЧ) || {}).data || null;
  ...
  (function обновитьЗапасы() {
    ...
    /*
     * Запрашиваем и тогда, когда нет списка «не везём».
     *
     * Он приезжает вместе с остатками, а остатки берутся из памяти браузера
     * на полчаса — из-за этого 17.08 список не приходил вовсе, и товары,
     * которые Ozon отказывается везти, оставались в каталоге.
     */
    if (!о || !НЕ_ВЕЗЁМ || Date.now() - о.when > ЗАПАС_СВЕЖЕСТЬ) {
      fetch(API + '/catalog/stock')
        .then(function (r) { return r.json(); })
        .then(function (j) {
          if (j && j.stock) { ОСТАТКИ_КАРТА = j.stock; вПамять(ОСТАТКИ_КЛЮЧ, j.stock); }
          // Товары, которые Ozon отказывается везти: продавать их нельзя,
          // отказ приходит уже после оплаты (17.08, BANNED_FOR_AREA).
          if (j && j.banned) { НЕ_ВЕЗЁМ = j.banned; вПамять(НЕВЕЗЁМ_КЛЮЧ, j.banned); }
        })
        .catch(function () {});
    }
  ...
  function ищутВКаталоге() {
    /*
     * Поиск считается начатым только после «Найти».
     *
     * Раньше сюда попадал и набираемый текст — и догрузка каталога снималась
     * с паузы прямо во время набора. Tilda на каждой догрузке пересобирает
     * панель фильтров вместе с полем, поэтому поле мигало и дёргалось
     * («что он моргает и лагает при наборе», 18.08). Пока запрос не применён,
     * витрина ведёт себя тихо.
     */
    return String(NGR_ЗАПРОС || '').trim().length >= 2;
  }
  ...
  function подмешатьСвои(текст, url) {
    ...
    var живые = список.filter(function (p) {
      /*
       * Товар, который Ozon не берётся везти, с витрины убираем.
       *
       * Он проходит и остаток, и цену, но на создании доставки приходит отказ
       * BANNED_FOR_AREA — уже после оплаты. Список приходит вместе с остатками.
       */
      if (НЕ_ВЕЗЁМ && НЕ_ВЕЗЁМ[String(p.sku)]) return false;
    ...
    /*
     * Обход закончен — отправляем воркеру перечень того, что Tilda отдала.
     *
     * Только по этому перечню он собирает полки и поиск: товар, которого
     * Tilda покупателю не отдаёт, она откажется и продать (отказ «осталось: 0»
     * на оформлении, 17.08). Сам воркер этот перечень получить не может —
     * Tilda отвечает ему отказом.
     */
    if (конецОбхода && onCatalogPage() && чистаяВыдача(url) && !window.NGR_ПЕРЕЧЕНЬ_ОТПРАВЛЕН) {
      window.NGR_ПЕРЕЧЕНЬ_ОТПРАВЛЕН = 1;
      var перечень = Object.keys(виденныеТовары);
      if (перечень.length > 300) {
        try {
          fetch(API + '/catalog/deliverable', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ skus: перечень })
          }).catch(function () {});
        } catch (e) {}
      }
    }
     ...
     * Дорисовка выключена.
     *
     * Она добавляла на витрину товары, которых Tilda не отдаёт, — а не отдаёт
     * она их потому, что в её базе у них нулевой остаток. Карточка появлялась,
     * «В корзину» работало, но на оформлении Tilda отказывала: «осталось: 0».
     * 17.08 Александр наткнулся на это дважды. Таких товаров 25 из 674 —
     * лучше их не показывать, чем вести покупателя в тупик.
     *
     * Включим обратно, когда остатки внутри Tilda будут обновляться: тогда
     * товары появятся у неё сами и станут покупаемыми.
     */
    var ДОРИСОВКА_ВКЛ = false;
    if (ДОРИСОВКА_ВКЛ && чистаяВыдача(url)) {
     ...
     */
    /*
     * Подстановка цены выключена.
     *
     * Tilda сверяет цену на своём сервере при оформлении. Наша цена 610 против
     * её 761 — и заказ отклоняется: «изменилась цена (новая цена: 761),
     * обновите страницу и добавьте его в корзину заново» (17.08, у Александра).
     * То есть показывать свою цену нельзя до тех пор, пока то же число не
     * лежит внутри Tilda. Цены надо заливать ей файлом, а не рисовать поверх.
     */
    var ПОДСТАВЛЯТЬ_ЦЕНЫ = false;
    if (ПОДСТАВЛЯТЬ_ЦЕНЫ && ЦЕНЫ_КАРТА) {
      живые.forEach(function (p) {
  ...
  function следитьЗаПоиском() {
    ...
    смотрительПоиска = new MutationObserver(function () {
      ...
      if (поле && !(поле.value || '').trim() && document.activeElement !== поле) {
        ...
        // Возвращаем только текст. Пересчитывать выдачу здесь нельзя: если
        // человек ещё набирает, каталог опустеет у него под руками.
        if (NGR_ЗАПРОС && typeof повторитьПоиск === 'function') повторитьПоиск();
        return;
      }
      /*
       ...
       */
      /*
       * Пересев — только для применённого запроса.
       *
       * Раньше условие смотрело на текст в поле, а его я запоминаю с первого
       * символа. Из-за этого каталог фильтровался прямо во время набора и
       * товары исчезали с экрана: «а сам товар пропадает в каталоге»
       * (Александр, 18.08). Пока человек не нажал «Найти», выдачи нет
       * и пересевать нечего.
       */
      var сейчас = document.querySelectorAll('#rec2502703571 .js-product').length;
      if (NGR_ЗАПРОС && сейчас !== карточекПриВыдаче) {
  ...
  function initSmartSearch() {
    ...
    function плашка(запрос, сколько, совпало) {
      ...
      /*
       * Пока каталог догружается, «ничего не нашли» — неправда.
       *
       * Замечание Александра 17.08: «сначала показывает не найдено, а потом
       * подгружает товар». Признак незаконченной загрузки — живая кнопка
       * «Загрузить ещё»: пока она есть, товары ещё идут, и вместо приговора
       * честнее сказать, что ищем. Пересев после догрузки уже настроен, так
       * что надпись сменится сама.
       */
      var ещёГрузится = !!document.querySelector('#rec2502703571 .js-catalog-load-more-btn');
      t.textContent = сколько
        ...
        : (совпало
            ...
            : (ещёГрузится
                ? ('Ищем «' + запрос + '» — каталог ещё загружается…')
    ...
    function отфильтровать() {
      ...
      /*
       * Поиск с главной уводит в каталог.
       *
       * На главной под рукой только короткая полка «В наличии сейчас», и поиск
       * по ней честно отвечал «ничего не нашли» на товар, который в каталоге
       * есть (замечание Александра 17.08 по артикулу 12933). Искать надо там,
       * где лежит весь ассортимент, поэтому переносим запрос в каталог.
       */
      if (сырое && !onCatalogPage()) {
        location.href = '/?catalog=all&ngrq=' + encodeURIComponent(сырое) + '#ngr-catalog';
        return;
      }
    ...
    /*
     * Запрос, пришедший с главной, применяем сразу.
     *
     * Поиск на главной уводит сюда с меткой в адресе. Каталог грузится
     * не мгновенно, поэтому пробуем несколько раз, пока карточки не появятся,
     * и только потом фильтруем — иначе покажем «ничего не нашли» на пустой
     * ещё витрине.
     */
    (function запросИзАдреса() {
      var м = location.search.match(/[?&]ngrq=([^&]*)/);
      if (!м) return;
      var зпр = '';
      try { зпр = decodeURIComponent(м[1].replace(/\+/g, ' ')); } catch (e) { зпр = м[1]; }
      зпр = String(зпр || '').trim();
      if (зпр.length < 2) return;
      НАБРАНО = зпр;
      var попыток = 0;
      (function попробовать() {
        попыток++;
        var поле = document.querySelector('#rec2502703571 .js-catalog-filter-search');
        var карточек = document.querySelectorAll('#rec2502703571 .js-product').length;
        if (поле && карточек > 30) {
          поле.value = зпр;
          отфильтровать();
          return;
        }
        if (попыток < 40) setTimeout(попробовать, 700);
      })();
    })();
    ...
    inp.addEventListener('input', function () {
      ...
      /*
       * Во время набора не ищем.
       *
       * Раньше на каждый символ пересчитывался указатель из 1340 товаров и
       * показывалась подсказка. На телефоне и на слабой машине это ощущалось
       * как зависание: «набираешь и сайт виснет» (Александр, 17.08). Поиск
       * теперь начинается только по кнопке «Найти» или по Enter — тогда же
       * рисуется и выдача в каталоге.
       */
      hide();
    });
    // По возвращению в поле подсказку тоже не показываем: она считается
    // по всему указателю и нужна только после нажатия «Найти».
