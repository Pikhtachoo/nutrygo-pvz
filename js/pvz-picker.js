 ...
 *
 * ВНИМАНИЕ. Этот файл лежит в ДВУХ репозиториях:
 *   Nutry/ozon-delivery/pvz-picker.js   — здесь; отсюда он вшивается в воркер
 *   nutrygo-pvz/js/pvz-picker.js        — оттуда его грузит сайт
 * Скрипт в head сайта берёт пикер сначала с GitHub Pages и только по onerror
 * из воркера. То есть правка ТОЛЬКО здесь до покупателей не доедет: 13.08.2026
 * так потерялись застава по телефону, подсказка про код получения и сверка с
 * профилем — их несколько дней не было на сайте, хотя код был написан
 * (журнал NG-2026-08-13-018).
 *
 * Правило: меняя этот файл, копируйте его в nutrygo-pvz/js/pvz-picker.js и
 * выкладывайте оба. Расхождение ловит проверка в деплое воркера — она сверяет
 * этот файл с тем, что реально отдаёт GitHub Pages, и не даёт выложиться.
 */
(function () {
  'use strict';
